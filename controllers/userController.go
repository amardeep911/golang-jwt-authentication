...
import (
	...
	"github.com/amardeep/golang-jwt-project/database"
	helper "github.com/amardeep/golang-jwt-project/helpers"
	"golang.org/x/crypto/bcrypt"

	"github.com/amardeep/golang-jwt-project/models"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while chekcing up the emails"})
		}
		...
		if err != nil {
			log.Panic(err)
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while chekcing up the phoneNo"})
		}
		if countEmail > 0 || countPhone > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "this email already exit"})
		}
		...
		if insertError != nil {
			msg := fmt.Sprintf("user item is not generated")
